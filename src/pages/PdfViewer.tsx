...
//   const validBookId: number = Number(bookId);

//   return (
//     <Frame className="bg-customGradient w-full h-[150vh]">
//       <CommunityTab bookId={validBookId} />
//       <AIChat />
...
const PdfViewer: React.FC = () => {
  ...
  const renderPage = async (pageNum: number) => {
    if (pdfDoc && canvasRef.current) {
      try {
        const page = await pdfDoc.getPage(pageNum);
        const scale = 1.5; // Adjust scale as needed for better visibility
        const viewport = page.getViewport({ scale });
        const context = canvasRef.current.getContext("2d");
        if (context) {
          canvasRef.current.height = viewport.height;
  ...
  return (
    <Frame>
      <CommunityTab bookId={Number(bookId) || 0} />
      <AIChat />
      ...
      <div className="flex flex-col items-center justify-center">
        ...
        <Progress value={(page / numPages) * 100} />
      </div>
    </Frame>
  );
};
